...
#define STR_SUCCESS "\033[1m\033[32m success\033[0m"
#define STR_FAILED "\033[1m\033[33m failed\033[0m"
...
private:
    ...
    void _log(LogLevel level [[maybe_unused]], std::stringstream& ss) {
        ss << '\n';
    }

    template <typename T, typename ...Args>
    void _log(LogLevel level, std::stringstream& ss, T&& first, Args&&... args) {
        ss << _log_color[0] << _log_color[static_cast<int>(level)] << first;
        _log(level, ss, std::forward<Args>(args)...);
    }
